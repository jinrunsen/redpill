#!/bin/bash
# redpill-session-state.sh — SessionStart hook: inject project state reminder
...
if [ -f .redpill/STATE.md ]; then
  echo 'STATE.md exists - check for blockers and current phase.'
  head -20 .redpill/STATE.md
else
  echo 'No .redpill/ found - suggest /redpill:new-project if starting new work.'
fi

echo ''

if [ -f .redpill/config.json ]; then
  MODE=$(grep -o '"mode"[[:space:]]*:[[:space:]]*"[^"]*"' .redpill/config.json 2>/dev/null || echo '"mode": "unknown"')
  echo "Config: $MODE"
fi
