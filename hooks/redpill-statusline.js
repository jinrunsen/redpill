...
process.stdin.on('end', () => {
  ...
  try {
    ...
    if (fs.existsSync(cacheFile)) {
      try {
        ...
        if (cache.update_available) {
          redpillUpdate = '\x1b[33m⬆ /redpill:update\x1b[0m │ ';
        }
        if (cache.stale_hooks && cache.stale_hooks.length > 0) {
          redpillUpdate += '\x1b[31m⚠ stale hooks — run /redpill:update\x1b[0m │ ';
        }
      } catch (e) {}
    }
    ...
    if (task) {
      process.stdout.write(`${redpillUpdate}\x1b[2m${model}\x1b[0m │ \x1b[1m${task}\x1b[0m │ \x1b[2m${dirname}\x1b[0m${ctx}`);
    } else {
      process.stdout.write(`${redpillUpdate}\x1b[2m${model}\x1b[0m │ \x1b[2m${dirname}\x1b[0m${ctx}`);
    }
  } catch (e) {
