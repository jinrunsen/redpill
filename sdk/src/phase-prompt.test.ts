...
describe('PromptFactory', () => {
  ...
  describe('buildPrompt', () => {
    it('assembles research prompt with role + purpose + process + context', async () => {
      ...
      await writeFile(
        join(agentsDir, 'redpill-phase-researcher.md'),
        makeAgentDef('redpill-phase-researcher', 'Read, Grep, Bash', 'You are a researcher.'),
      );
    ...
    it('assembles plan prompt with all context files', async () => {
      ...
      await writeFile(
        join(agentsDir, 'redpill-planner.md'),
        makeAgentDef('redpill-planner', 'Read, Write, Bash', 'You are a planner.'),
      );
    ...
    it('delegates execute phase with plan to buildExecutorPrompt', async () => {
      await writeFile(
        join(agentsDir, 'redpill-executor.md'),
        makeAgentDef('redpill-executor', 'Read, Write, Edit, Bash', 'You are an executor.'),
      );
    ...
    it('assembles verify prompt with phase instructions', async () => {
      ...
      await writeFile(
        join(agentsDir, 'redpill-verifier.md'),
        makeAgentDef('redpill-verifier', 'Read, Bash, Grep', 'You are a verifier.'),
      );
    ...
    it('handles missing workflow file gracefully', async () => {
      ...
      await writeFile(
        join(agentsDir, 'redpill-phase-researcher.md'),
        makeAgentDef('redpill-phase-researcher', 'Read, Bash', 'You are a researcher.'),
      );
  ...
  describe('loadAgentDef', () => {
    ...
    it('falls back to project agents dir', async () => {
      ...
      await writeFile(
        join(projectAgentsDir, 'redpill-executor.md'),
    ...
    it('prefers user agents dir over project agents dir', async () => {
      const projectAgentsDir = join(tempDir, 'project-agents');
      await mkdir(projectAgentsDir, { recursive: true });
      await writeFile(join(agentsDir, 'redpill-executor.md'), 'user agent');
      await writeFile(join(projectAgentsDir, 'redpill-executor.md'), 'project agent');
  ...
  describe('headless prompt loading', () => {
    ...
    it('loadAgentDef prefers sdkPromptsDir over user agents dir', async () => {
      ...
      await writeFile(join(agentsDir, 'redpill-executor.md'), 'user agent');
      await writeFile(join(sdkDir, 'agents', 'redpill-executor.md'), 'SDK headless agent');
    ...
    it('buildPrompt sanitizes interactive patterns from output', async () => {
      ...
      expect(prompt).not.toContain('/redpill:');
