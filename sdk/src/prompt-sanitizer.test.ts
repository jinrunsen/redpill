...
describe('sanitizePrompt', () => {
  ...
  describe('@file: references', () => {
    ...
    it('strips @file: with various path formats', () => {
      const input = [
        ...
        '@file:~/.claude/redpill/workflows/execute-plan.md',
      ].join('\n');
  ...
  describe('/redpill: slash commands', () => {
    it('strips lines containing /redpill: commands', () => {
      const input = 'Before\nRun /redpill:execute-plan to proceed\nAfter';
      const result = sanitizePrompt(input);
      expect(result).not.toContain('/redpill:');
      expect(result).toContain('Before');
      expect(result).toContain('After');
    });

    it('strips various /redpill: command formats', () => {
      const input = [
        'Use /redpill:research-phase',
        'Then /redpill:plan-phase --auto',
        'Finally /redpill:verify-phase',
      ].join('\n');
      expect(sanitizePrompt(input)).toBe('');
    });
  });
  ...
  describe('multiple patterns in one string', () => {
    it('strips all pattern types from a mixed prompt', () => {
      const input = [
        ...
        '',
        'When done, run /redpill:plan-phase to proceed.',
        '',
