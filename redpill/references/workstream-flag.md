...
The `--ws <name>` flag scopes REDPILL operations to a specific workstream, enabling
parallel milestone work by multiple Claude Code instances on the same codebase.

## Resolution Priority

1. `--ws <name>` flag (explicit, highest priority)
2. `REDPILL_WORKSTREAM` environment variable (per-instance)
3. `.redpill/active-workstream` file (shared, last-writer-wins)
4. `null` — flat mode (no workstreams)

## Routing Propagation

All workflow routing commands include `${REDPILL_WS}` which:
...
# Workstream CRUD
node redpill-tools.cjs workstream create <name>
node redpill-tools.cjs workstream list
node redpill-tools.cjs workstream status <name>
node redpill-tools.cjs workstream complete <name>
```
