<purpose>
Create all phases necessary to close gaps identified by `/redpill:audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/redpill:add-phase` per gap.
</purpose>
...
(ls -t .redpill/v*-MILESTONE-AUDIT.md 2>/dev/null || true) | head -1
```
...
```
No audit gaps found. Run `/redpill:audit-milestone` first.
```
...
HIGHEST=$(node "$HOME/.claude/redpill/bin/redpill-tools.cjs" phases list --pick directories[-1])
```
...
grep -c "Pending" .redpill/REQUIREMENTS.md
```

## 8. Create Phase Directories

```bash
mkdir -p ".redpill/phases/{NN}-{name}"
```

## 9. Commit Roadmap and Requirements Update

```bash
node "$HOME/.claude/redpill/bin/redpill-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .redpill/ROADMAP.md .redpill/REQUIREMENTS.md
```
...
**Also available:**
- `/redpill:execute-phase {N}` — if plans already exist
- `cat .redpill/ROADMAP.md` — see updated roadmap

---

**After all gap phases complete:**

`/redpill:audit-milestone` — re-audit to verify gaps closed
`/redpill:complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/redpill:plan-phase` next
</success_criteria>
