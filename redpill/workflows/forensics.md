...
Post-mortem investigation for failed or stuck REDPILL workflows. Analyzes git history,
`.redpill/` artifacts, and file system state to detect anomalies and generate a
...
- `.redpill/STATE.md` — current milestone, phase, progress, blockers, last session
- `.redpill/ROADMAP.md` — phase list with status
- `.redpill/config.json` — workflow configuration
...
Read `.redpill/reports/SESSION_REPORT.md` if it exists — extract last session outcomes,
...
```bash
mkdir -p .redpill/forensics
```

Write to `.redpill/forensics/report-$(date +%Y%m%d-%H%M%S).md`:
...
3. {Recovery command if applicable — e.g., `/redpill:resume-work`, `/redpill:execute-phase N`}

---

*Report generated by `/redpill:forensics`. All paths redacted for portability.*
```
...
> "Report saved to `.redpill/forensics/report-{timestamp}.md`.
>
...
redpill-tools.cjs state record-session \
  ...
  --resume-file ".redpill/forensics/report-{timestamp}.md"
```
