<purpose>
Remove a REDPILL workspace, cleaning up git worktrees and deleting the workspace directory.
</purpose>
...
```bash
INIT=$(node "$HOME/.claude/redpill/bin/redpill-tools.cjs" init remove-workspace "$WORKSPACE_NAME")
...
First run `/redpill:list-workspaces` to show available workspaces, then ask:
