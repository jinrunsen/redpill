...
loop:
  ...
  # 阶段 2: WORK — 检测 undefined steps（必须执行，不可跳过）

  ## 2a. 运行 behave dry-run 检测 undefined steps

  ```bash
  behave --dry-run --no-capture --format plain --include <feature_file> 2>&1
  ```

  解析输出，检查是否存在 undefined steps。判断条件（满足任一即为 undefined）：
  - 输出包含 "NotImplementedError" 或 "not yet implemented"
  - 输出包含 "You can implement step definitions"
  - 步骤状态统计中有 "undefined"
  - 输出包含 "Undefined step"

  ## 2b. 如果有 undefined steps → 必须派发 redpill-step-writer

  **这是硬性要求。检测到 undefined steps 时，必须使用 Agent 工具派发 redpill-step-writer 子代理。
  绝不允许跳过此步直接进入 implementer。implementer 不写 step 代码。**

  使用 Agent 工具：
  - subagent_type: "redpill-step-writer"
  - prompt 中必须包含：
    1. 当前场景的完整 Gherkin 文本（从 .feature 文件读取）
    2. feature 文件路径
    3. API 契约路径（.redpill/wip/api/ 下的文件）
    4. 已有的 step 文件路径（features/steps/*.py）
    5. 技术设计文档路径（.redpill/wip/designs/ 下的文件）
    6. behave dry-run 的输出（让 step-writer 知道哪些 step 缺失）

  等待 agent 完成。

  ## 2c. 验证 step 已定义

  重新运行 behave dry-run，确认不再有 undefined steps。
  如果仍有 undefined → 最多重试 1 次（再次派发 step-writer），之后报错退出。

  ## 2d. 如果没有 undefined steps → 直接进入阶段 3

  所有 step 已定义，失败原因在生产代码 → 跳到阶段 3。

  # 阶段 3: 实现场景

  **前置条件：所有 step 必须已定义（阶段 2 已确认），否则不得进入此阶段。**
  ...
  使用 Agent 工具派发 redpill-implementer：
  - subagent_type: "redpill-implementer"
  - prompt 中必须包含：
    1. scenario — 当前场景的 Gherkin 文本
    2. steps — step 定义文件内容（实际代码，非路径）
    3. design — 技术设计文档内容
    4. conventions — 代码约定
    5. behave_output — 最新的 behave 测试输出

  等待 agent 完成，收集代码变更。

  # 阶段 4: 审查
  spawn Agent(redpill-scenario-reviewer)
    注入: @redpill/skills/subagent-driven-development/scenario-reviewer-prompt.md
    上下文: { scenario, code_changes }

  spawn Agent(redpill-quality-reviewer)
    注入: @redpill/skills/subagent-driven-development/quality-reviewer-prompt.md
