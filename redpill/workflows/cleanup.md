...
Archive accumulated phase directories from completed milestones into `.redpill/milestones/v{X.Y}-phases/`. Identifies which phases belong to each completed milestone, shows a dry-run summary, and moves directories on confirmation.

</purpose>

<required_reading>

1. `.redpill/MILESTONES.md`
2. `.redpill/milestones/` directory listing
3. `.redpill/phases/` directory listing
...
Read `.redpill/MILESTONES.md` to identify completed milestones and their versions.

```bash
cat .redpill/MILESTONES.md
```
...
```bash
ls -d .redpill/milestones/v*-phases 2>/dev/null || true
```
...
```bash
cat .redpill/milestones/v{X.Y}-ROADMAP.md
```

Extract phase numbers and names from the archived roadmap (e.g., Phase 1: Foundation, Phase 2: Auth).

Check which of those phase directories still exist in `.redpill/phases/`:

```bash
ls -d .redpill/phases/*/ 2>/dev/null || true
```

Match phase directories to milestone membership. Only include directories that still exist in `.redpill/phases/`.
...
Destination: .redpill/milestones/v{X.Y}-phases/
...
Destination: .redpill/milestones/v{X.Z}-phases/
```
...
```bash
mv .redpill/phases/{dir} .redpill/milestones/v{X.Y}-phases/
```
...
```bash
node "$HOME/.claude/redpill/bin/redpill-tools.cjs" commit "chore: archive phase directories from completed milestones" --files .redpill/milestones/ .redpill/phases/
```
...
Archived:
{For each milestone}
- v{X.Y}: {N} phase directories → .redpill/milestones/v{X.Y}-phases/

.redpill/phases/ cleaned up.
```
...
- [ ] Phase directories moved to `.redpill/milestones/v{X.Y}-phases/`
- [ ] Changes committed
