...
<required_reading>
@~/.claude/redpill/references/verification-patterns.md
@~/.claude/redpill/templates/verification-report.md
</required_reading>
...
grep -E "^| ${phase_number}" .redpill/REQUIREMENTS.md 2>/dev/null || true
...
Use redpill-tools to extract must_haves from each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node "$HOME/.claude/redpill/bin/redpill-tools.cjs" frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node "$HOME/.claude/redpill/bin/redpill-tools.cjs" roadmap get-phase "${phase_number}" --raw)
```
...
Use redpill-tools for artifact verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node "$HOME/.claude/redpill/bin/redpill-tools.cjs" verify artifacts "$plan")
...
Use redpill-tools for key link verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node "$HOME/.claude/redpill/bin/redpill-tools.cjs" verify key-links "$plan")
...
```bash
grep -E "Phase ${PHASE_NUM}" .redpill/REQUIREMENTS.md 2>/dev/null || true
```
...
See ~/.claude/redpill/templates/verification-report.md for complete template.
</step>
