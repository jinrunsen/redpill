...
```bash
INIT=$(node "$HOME/.claude/redpill/bin/redpill-tools.cjs" init milestone-op)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
```

Parse JSON for: `milestone_version`, `milestone_name`, `phase_count`, `completed_phases`, `roadmap_exists`, `state_exists`, `commit_docs`.

**If `roadmap_exists` is false:** Error — "No ROADMAP.md found. Run `/redpill:new-milestone` first."
**If `state_exists` is false:** Error — "No STATE.md found. Run `/redpill:new-milestone` first."
...
```bash
DETAIL=$(node "$HOME/.claude/redpill/bin/redpill-tools.cjs" roadmap get-phase ${PHASE_NUM})
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 REDPILL ► AUTONOMOUS ▸ Phase {N}/{T}: {Name} [████░░░░] {P}%
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
```bash
PHASE_STATE=$(node "$HOME/.claude/redpill/bin/redpill-tools.cjs" init phase-op ${PHASE_NUM})
```
...
```bash
SKIP_DISCUSS=$(node "$HOME/.claude/redpill/bin/redpill-tools.cjs" config-get workflow.skip_discuss 2>/dev/null || echo "false")
```
...
```bash
node "$HOME/.claude/redpill/bin/redpill-tools.cjs" commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_SECTION=$(node "$HOME/.claude/redpill/bin/redpill-tools.cjs" roadmap get-phase ${PHASE_NUM} 2>/dev/null)
...
```
Skill(skill="redpill:ui-phase", args="${PHASE_NUM}")
```
...
Agent(
  ...
  prompt="Run plan-phase for phase ${PHASE_NUM}: Skill(skill=\"redpill:plan-phase\", args=\"${PHASE_NUM}\")"
)
```
...
```
Skill(skill="redpill:plan-phase", args="${PHASE_NUM}")
```
...
Agent(
  ...
  run_in_background=true,
  prompt="Run execute-phase for phase ${PHASE_NUM}: Skill(skill=\"redpill:execute-phase\", args=\"${PHASE_NUM} --no-transition\")"
)
```
...
```
Skill(skill="redpill:execute-phase", args="${PHASE_NUM} --no-transition")
```
...
```
Skill(skill="redpill:plan-phase", args="${PHASE_NUM} --gaps")
```
...
```bash
UI_REVIEW_CFG=$(node "$HOME/.claude/redpill/bin/redpill-tools.cjs" config-get workflow.ui_review 2>/dev/null || echo "true")
```
...
```
Skill(skill="redpill:ui-review", args="${PHASE_NUM}")
```
...
> **Note:** Smart discuss is an autonomous-optimized variant of the `redpill:discuss-phase` skill. It produces identical CONTEXT.md output but uses batch table proposals instead of sequential questioning. The original `discuss-phase` skill remains unchanged (per CTRL-03). Future milestones may extract this to a separate skill file.
...
```bash
node "$HOME/.claude/redpill/bin/redpill-tools.cjs" commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 REDPILL ► AUTONOMOUS ▸ PHASE ${ONLY_PHASE} COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

 Phase ${ONLY_PHASE}: ${PHASE_NAME} — Done
 Mode: Single phase (--only)

 Lifecycle skipped — run /redpill:autonomous without --only
...
```
Skill(skill="redpill:audit-milestone")
```

After audit completes, detect the result:

```bash
AUDIT_FILE=".redpill/v${milestone_version}-MILESTONE-AUDIT.md"
...
On **"Stop"**: Go to handle_blocker with "User stopped — audit gaps remain. Run /redpill:audit-milestone to review, then /redpill:complete-milestone when ready."
...
On **"Stop"**: Go to handle_blocker with "User stopped — tech debt to address. Run /redpill:audit-milestone to review details."

**5b. Complete Milestone**

```
Skill(skill="redpill:complete-milestone", args="${milestone_version}")
```

After complete-milestone returns, verify it produced output:

```bash
ls .redpill/milestones/v${milestone_version}-ROADMAP.md 2>/dev/null || true
```
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume with: /redpill:autonomous ${ONLY_PHASE ? "--only " + ONLY_PHASE : "--from " + next_phase}
```
...
- [ ] Audit invoked via Skill(skill="redpill:audit-milestone")
...
- [ ] `--interactive` runs discuss inline via redpill:discuss-phase (asks questions, waits for user)
