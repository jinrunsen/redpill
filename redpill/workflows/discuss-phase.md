...
1. **redpill-phase-researcher** — Reads CONTEXT.md to know WHAT to research
   - "User wants card-based layout" → researcher investigates card component patterns
   - "Infinite scroll decided" → researcher looks into virtualization libraries

2. **redpill-planner** — Reads CONTEXT.md to know WHAT decisions are locked
...
**Express path available:** If you already have a PRD or acceptance criteria document, use `/redpill:plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.
...
AGENT_SKILLS_ADVISOR=$(node "$HOME/.claude/redpill/bin/redpill-tools.cjs" agent-skills redpill-advisor 2>/dev/null)
```

Parse JSON for: `commit_docs`, `phase_found`, `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`, `has_research`, `has_context`, `has_plans`, `has_verification`, `plan_count`, `roadmap_exists`, `redpill_dir_exists`.

**If `phase_found` is false:**
```
Phase [X] not found in roadmap.

Use /redpill:progress ${REDPILL_WS} to see available phases.
```
Exit workflow.
...
- options:
  - "Continue and replan after" — Capture context, then run /redpill:plan-phase {X} ${REDPILL_WS} to replan
...
cat .redpill/REQUIREMENTS.md 2>/dev/null || true
...
(find .redpill/phases -name "*-CONTEXT.md" 2>/dev/null || true) | sort
```
...
```bash
TODO_MATCHES=$(node "$HOME/.claude/redpill/bin/redpill-tools.cjs" todo match-phase "${PHASE_NUMBER}")
```
...
```bash
ls .redpill/codebase/*.md 2>/dev/null || true
```
...
1. Check for USER-PROFILE.md:
   ```bash
   PROFILE_PATH="$HOME/.claude/redpill/USER-PROFILE.md"
   ```
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(node "$HOME/.claude/redpill/bin/redpill-tools.cjs" resolve-model redpill-advisor-researcher --raw)
   ```
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @~/.claude/agents/redpill-advisor-researcher.md for your role and instructions.
...
**Research-before-questions mode:** Check if `workflow.research_before_questions` is enabled in config (from init context or `.redpill/config.json`). When enabled, before presenting questions for each area:
...
```bash
MAX_PASSES=$(node "$HOME/.claude/redpill/bin/redpill-tools.cjs" config-get workflow.max_discuss_passes 2>/dev/null || echo "3")
```
...
This is a structured checkpoint, not the final CONTEXT.md — the `write_context` step still produces the canonical output. But if the session dies, the next `redpill:discuss-phase` invocation can detect this checkpoint and offer to resume from it instead of starting from scratch.
...
```
Created: .redpill/phases/${PADDED_PHASE}-${SLUG}/${PADDED_PHASE}-CONTEXT.md
...
`/redpill:plan-phase ${PHASE} ${REDPILL_WS}`
...
**Also available:**
- `/redpill:discuss-phase ${PHASE} --chain ${REDPILL_WS}` — re-run with auto plan+execute after
- `/redpill:plan-phase ${PHASE} --skip-research ${REDPILL_WS}` — plan without research
- `/redpill:ui-phase ${PHASE} ${REDPILL_WS}` — generate UI design contract before planning (if phase has frontend work)
...
```bash
node "$HOME/.claude/redpill/bin/redpill-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
node "$HOME/.claude/redpill/bin/redpill-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .redpill/STATE.md
```
</step>
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 REDPILL ► AUTO-ADVANCING TO PLAN
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```
Skill(skill="redpill:plan-phase", args="${PHASE} --auto ${REDPILL_WS}")
```
...
- **PHASE COMPLETE** → Full chain succeeded. Display:
  ...
  Next: /redpill:discuss-phase ${NEXT_PHASE} ${WAS_CHAIN ? "--chain" : "--auto"} ${REDPILL_WS}
  <sub>/clear first → fresh context window</sub>
  ```
- **PLANNING COMPLETE** → Planning done, execution didn't complete:
  ```
  Auto-advance partial: Planning complete, execution did not finish.
  Continue: /redpill:execute-phase ${PHASE} ${REDPILL_WS}
  ```
- **PLANNING INCONCLUSIVE / CHECKPOINT** → Stop chain:
  ```
  Auto-advance stopped: Planning needs input.
  Continue: /redpill:plan-phase ${PHASE} ${REDPILL_WS}
  ```
- **GAPS FOUND** → Stop chain:
  ```
  Auto-advance stopped: Gaps found during execution.
  Continue: /redpill:plan-phase ${PHASE} --gaps ${REDPILL_WS}
  ```
