...
Use /redpill:quick for anything that needs multi-step planning or research.
</purpose>
...
```
This looks like it needs planning. Use /redpill:quick instead:
  /redpill:quick "{task description}"
```
...
If `.redpill/STATE.md` exists, append to the "Quick Tasks Completed" table.
If the table doesn't exist, skip this step silently.

```bash
# Check if STATE.md has quick tasks table
if grep -q "Quick Tasks Completed" .redpill/STATE.md 2>/dev/null; then
  # Append entry — workflow handles the format
  echo "| $(date +%Y-%m-%d) | fast | $TASK | ✅ |" >> .redpill/STATE.md
fi
```
</step>
...
- If the task takes more than 3 file edits, STOP and redirect to /redpill:quick
- If you're unsure how to implement it, STOP and redirect to /redpill:quick
</guardrails>
