<purpose>
Generate a post-session summary document capturing work performed, outcomes achieved, and estimated resource usage. Writes SESSION_REPORT.md to .redpill/reports/ for human review and stakeholder sharing.
</purpose>
...
Read `.redpill/STATE.md` to get:
- Current milestone and phase
- Progress percentage
- Active blockers
- Recent decisions

Read `.redpill/ROADMAP.md` to get milestone name and goals.

Check for existing reports:
```bash
ls -la .redpill/reports/SESSION_REPORT*.md 2>/dev/null || echo "No previous reports"
```
</step>
...
Write `.redpill/reports/SESSION_REPORT.md` (or `.redpill/reports/YYYYMMDD-session-report.md` if previous reports exist):
...
*Generated by `/redpill:session-report`*
```
</step>
...
```
💡 Run `/redpill:session-report` at the end of each session to build a history of project activity.
```
</step>
...
- [ ] Report written to .redpill/reports/
