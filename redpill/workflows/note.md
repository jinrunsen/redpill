...
- **Global scope**: `~/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.redpill/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.redpill/notes/` or `~/.claude/notes/`)
...
6. Generate todo ID: `{NNN}-{slug}` where NNN is the next sequential number (scan both `.redpill/todos/pending/` and `.redpill/todos/completed/` for the highest existing number, increment by 1, zero-pad to 3 digits) and slug is the first ~4 meaningful words of the note text
...
2. **No `.redpill/`**: Falls back to global `~/.claude/notes/` — works in any directory
