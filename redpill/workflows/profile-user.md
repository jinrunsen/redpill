...
This workflow wires Phase 1 (session pipeline) and Phase 2 (profiling engine) into a cohesive user-facing experience. All heavy lifting is done by existing redpill-tools.cjs subcommands and the redpill-user-profiler agent -- this workflow orchestrates the sequence, handles branching, and provides the UX.
</purpose>
...
Key references:
- @$HOME/.claude/redpill/references/ui-brand.md (display patterns)
- @$HOME/.claude/redpill/agents/redpill-user-profiler.md (profiler agent definition)
- @$HOME/.claude/redpill/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
cp "$HOME/.claude/redpill/USER-PROFILE.md" "$HOME/.claude/redpill/USER-PROFILE.backup.md"
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 REDPILL > PROFILE YOUR CODING STYLE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
✓ Stores profile at $HOME/.claude/redpill/USER-PROFILE.md
...
- options:
  ...
  - "Not now" -- Display "No worries. Run /redpill:profile-user when ready." and exit
...
Run session scan:
```bash
SCAN_RESULT=$(node $HOME/.claude/redpill/bin/redpill-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node $HOME/.claude/redpill/bin/redpill-tools.cjs profile-sample --json 2>/dev/null)
```
...
**Spawn redpill-user-profiler agent using Task tool:**

Use the Task tool to spawn the `redpill-user-profiler` agent. Provide it with:
- The sampled JSONL file path from profile-sample output
- The user-profiling reference doc at `$HOME/.claude/redpill/references/user-profiling.md`
...
Reference: @$HOME/.claude/redpill/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node $HOME/.claude/redpill/bin/redpill-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(node $HOME/.claude/redpill/bin/redpill-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
node $HOME/.claude/redpill/bin/redpill-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to $HOME/.claude/redpill/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  - "/redpill:dev-preferences command file" -- "Load your preferences in any session"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at $HOME/.claude/redpill/USER-PROFILE.md" and jump to step 10.
...
**For /redpill:dev-preferences (if selected):**
...
```bash
node $HOME/.claude/redpill/bin/redpill-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node $HOME/.claude/redpill/bin/redpill-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```

Display: "✓ Added profile section to $HOME/.claude/CLAUDE.md"

**Error handling:** If any redpill-tools.cjs call fails, display the error message and use AskUserQuestion to offer "Retry" or "Skip this artifact". On retry, re-run the command. On skip, continue to next artifact.
...
```bash
BACKUP_PATH="$HOME/.claude/redpill/USER-PROFILE.backup.md"
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 REDPILL > PROFILE COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Your profile:    $HOME/.claude/redpill/USER-PROFILE.md
```
...
- [ ] Artifacts generated sequentially via redpill-tools.cjs subcommands
