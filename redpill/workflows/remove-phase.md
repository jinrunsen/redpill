...
- Example: `/redpill:remove-phase 17` → phase = 17
- Example: `/redpill:remove-phase 16.1` → phase = 16.1

If no argument provided:

```
ERROR: Phase number required
Usage: /redpill:remove-phase <phase-number>
Example: /redpill:remove-phase 17
```
...
```bash
INIT=$(node "$HOME/.claude/redpill/bin/redpill-tools.cjs" init phase-op "${target}")
...
To abandon current work, use /redpill:pause-work instead.
```
...
This will:
- Delete: .redpill/phases/{target}-{slug}/
...
```bash
node "$HOME/.claude/redpill/bin/redpill-tools.cjs" commit "chore: remove phase {target} ({original-phase-name})" --files .redpill/
```
...
Changes:
- Deleted: .redpill/phases/{target}-{slug}/
...
Would you like to:
- `/redpill:progress` — see updated roadmap status
