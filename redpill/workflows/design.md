...
- redpill-tech-reviewer — Reviews technical design for scenario coverage, codebase
  consistency, over-design detection, pattern conformance, and autonomy fitness.
  The sole quality gate before implementation begins. Read-only.
</available_agent_types>
...
**NOTE:** There is no `init design` handler. Use `init clarify-feature`
...
`init design` — it does not exist and will error.
...
while REVIEW_ROUND <= REVIEW_MAX:
  ...
  Agent(
    subagent_type="redpill-tech-reviewer",
    model="${verifier_model}",
    description="Tech review design: ${SLUG} (round ${REVIEW_ROUND})",
    prompt="
      <objective>
      Review the technical design document. You are the sole quality gate
      before implementation begins. Perform BOTH Part 1 (standard quality)
      and Part 2 (autonomous audit) from your agent definition.

      Read actual code files to verify pattern claims — do not trust the
      design's descriptions of existing patterns.
      </objective>
      ...
      <output_contract>
      Return a <TECH_DESIGN_REVIEW> block as specified in your agent definition.
      Every issue MUST have a 'category' field (auto-fixable | product-decision).
      </output_contract>
    "
  )

  Parse <TECH_DESIGN_REVIEW> block. Extract verdict.
  ...
  if verdict == NEEDS_HUMAN_DESIGN:
    Display:
    ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
     REDPILL ► DESIGN PAUSED — needs human review
    ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

     Reason: ${summary}
     Design draft preserved at: ${DESIGN_PATH}

     Next: review and refine the design manually, then re-run
           /redpill:design-feature ${FEATURE_ARG} --skip-review
    Exit (do not continue).

  if verdict == NEEDS_REVISION:
    Split issues into tech_issues (auto-fixable) and product_issues (product-decision).

    **Interactive mode:** Present all issues to user via AskUserQuestion,
    let them choose which to fix and how.

    **Auto mode:** Auto-fix auto-fixable issues by editing the design document.
