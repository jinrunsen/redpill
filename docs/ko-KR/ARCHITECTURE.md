...
워크플로우 파일(`redpill/workflows/*.md`)은 무거운 작업을 직접 수행하지 않습니다. 다음 작업만 담당합니다.
- `redpill-tools.cjs init <workflow>`로 컨텍스트를 로드합니다
...
모든 상태는 `.redpill/`에 사람이 읽을 수 있는 Markdown과 JSON으로 저장됩니다. 데이터베이스, 서버, 외부 의존성이 없습니다. 이를 통해 다음이 가능합니다.
...
- `redpill-tools.cjs init`을 통한 컨텍스트 로드
...
모든 계획 아티팩트를 위한 Markdown 템플릿입니다. `redpill-tools.cjs template fill`과 `scaffold` 명령어가 사전 구조화된 파일을 생성하는 데 사용합니다.
...
| 훅 | 이벤트 | 목적 |
|------|-------|---------|
| `redpill-statusline.js` | `statusLine` | 모델, 작업, 디렉터리, 컨텍스트 사용 바 표시 |
| `redpill-context-monitor.js` | `PostToolUse` / `AfterTool` | 잔여 35%/25% 시점에 에이전트 대면 컨텍스트 경고 주입 |
| `redpill-check-update.js` | `SessionStart` | 새 REDPILL 버전을 백그라운드에서 확인 |
| `redpill-prompt-guard.js` | `PreToolUse` | `.redpill/` 쓰기 작업에서 프롬프트 인젝션 패턴 스캔 (권고용) |
| `redpill-workflow-guard.js` | `PreToolUse` | REDPILL 워크플로우 컨텍스트 외부의 파일 편집 감지 (권고용, `hooks.workflow_guard`로 활성화) |

### CLI Tools (`redpill/bin/`)

17개의 도메인 모듈을 포함하는 Node.js CLI 유틸리티(`redpill-tools.cjs`)입니다.
...
| 카테고리 | 에이전트 | 병렬성 |
|----------|--------|-------------|
| **Researchers** | redpill-project-researcher, redpill-phase-researcher, redpill-ui-researcher, redpill-advisor-researcher | 4개 병렬 (stack, features, architecture, pitfalls); advisor는 discuss-phase 중 생성됨 |
| **Synthesizers** | redpill-research-synthesizer | 순차적 (조사자 완료 후) |
| **Planners** | redpill-planner, redpill-roadmapper | 순차적 |
| **Checkers** | redpill-plan-checker, redpill-integration-checker, redpill-ui-checker, redpill-nyquist-auditor | 순차적 (검증 루프, 최대 3회 반복) |
| **Executors** | redpill-executor | 웨이브 내 병렬, 웨이브 간 순차적 |
| **Verifiers** | redpill-verifier | 순차적 (모든 executor 완료 후) |
| **Mappers** | redpill-codebase-mapper | 4개 병렬 (tech, arch, quality, concerns) |
| **Debuggers** | redpill-debugger | 순차적 (대화형) |
| **Auditors** | redpill-ui-auditor | 순차적 |
...
├── hooks/
│   ├── redpill-statusline.js           # 상태표시줄 훅
│   ├── redpill-context-monitor.js      # 컨텍스트 경고 훅
│   └── redpill-check-update.js         # 업데이트 확인 훅
├── settings.json                   # 훅 등록
└── VERSION                         # 설치된 버전 번호
```
...
├── research/               # /redpill:new-project의 도메인 조사
│   ├── SUMMARY.md
│   ├── STACK.md
│   ├── FEATURES.md
│   ├── ARCHITECTURE.md
│   └── PITFALLS.md
├── codebase/               # 브라운필드 매핑 (/redpill:map-codebase에서)
│   ├── STACK.md
│   ├── ARCHITECTURE.md
│   ├── CONVENTIONS.md
│   ├── CONCERNS.md
...
│   ├── pending/            # 캡처된 아이디어
│   └── done/               # 완료된 할 일
├── threads/               # 영구 컨텍스트 스레드 (/redpill:thread에서)
├── seeds/                 # 미래 지향적 아이디어 (/redpill:plant-seed에서)
├── debug/                  # 활성 디버그 세션
│   ├── *.md                # 활성 세션
│   ├── resolved/           # 보관된 세션
│   └── knowledge-base.md   # 영구 디버그 학습 내용
├── ui-reviews/             # /redpill:ui-review의 스크린샷 (gitignored)
...
- `.redpill/` 파일에 Write/Edit 시 트리거됩니다
...
- `.redpill/` 외부 파일에 Write/Edit 시 트리거됩니다
- REDPILL 워크플로우 컨텍스트 외부의 편집을 감지합니다 (활성 `/redpill:` 명령어 또는 Task 서브에이전트 없음)
- 상태 추적 변경을 위해 `/redpill:quick` 또는 `/redpill:fast` 사용을 권고합니다
