...
**사전 조건:** `.redpill/PROJECT.md`가 존재하지 않아야 합니다.
**생성 파일:** `PROJECT.md`, `REQUIREMENTS.md`, `ROADMAP.md`, `STATE.md`, `config.json`, `research/`, `CLAUDE.md`

```bash
/redpill:new-project                    # 대화형 모드
/redpill:new-project --auto @prd.md     # PRD에서 자동 추출
```
...
격리된 워크스페이스를 생성합니다. 저장소 복사본과 독립적인 `.redpill/` 디렉터리가 포함됩니다.
...
**사용 사례.**
- 멀티 저장소: 격리된 REDPILL 상태로 일부 저장소만 작업합니다.
- 기능 격리: `--repos .`는 현재 저장소의 worktree를 생성합니다.

**생성 파일:** `WORKSPACE.md`, `.redpill/`, 저장소 복사본 (worktree 또는 clone)

```bash
/redpill:new-workspace --name feature-b --repos hr-ui,ZeymoAPI
/redpill:new-workspace --name feature-b --repos . --strategy worktree  # 동일 저장소 격리
/redpill:new-workspace --name spike --repos api,web --strategy clone   # 전체 클론
```
...
**사전 조건:** `.redpill/ROADMAP.md`가 존재해야 합니다.
**생성 파일:** `{phase}-CONTEXT.md`, `{phase}-DISCUSSION-LOG.md` (감사 추적)

```bash
/redpill:discuss-phase 1                # 페이즈 1 대화형 토론
/redpill:discuss-phase 3 --auto         # 페이즈 3 기본값 자동 선택
/redpill:discuss-phase --batch          # 현재 페이즈 일괄 모드
/redpill:discuss-phase 2 --analyze      # 트레이드오프 분석 포함 토론
```
...
**사전 조건:** `.redpill/ROADMAP.md`가 존재해야 하며 해당 페이즈에 프론트엔드/UI 작업이 포함되어야 합니다.
**생성 파일:** `{phase}-UI-SPEC.md`

```bash
/redpill:ui-phase 2                     # 페이즈 2 설계 계약 생성
```
...
```bash
/redpill:plan-phase 1                   # 페이즈 1 조사 + 계획 + 검증
/redpill:plan-phase 3 --skip-research   # 조사 없이 계획 (익숙한 도메인)
/redpill:plan-phase --auto              # 비대화형 계획 수립
```
...
```bash
/redpill:execute-phase 1                # 페이즈 1 실행
/redpill:execute-phase 1 --wave 2       # Wave 2만 실행
```
...
```bash
/redpill:verify-work 1                  # 페이즈 1 UAT
```
...
**사전 조건:** `.redpill/` 디렉터리가 존재해야 합니다.
**동작 방식.**
- 프로젝트 없음 → `/redpill:new-project` 제안
- 페이즈 토론 필요 → `/redpill:discuss-phase` 실행
- 페이즈 계획 필요 → `/redpill:plan-phase` 실행
- 페이즈 실행 필요 → `/redpill:execute-phase` 실행
- 페이즈 검증 필요 → `/redpill:verify-work` 실행
...
**생성 파일:** `.redpill/reports/SESSION_REPORT.md`

```bash
/redpill:session-report                 # 세션 종료 후 요약 생성
```
...
**사전 조건:** 페이즈 검증 완료 (`/redpill:verify-work` 통과), `gh` CLI 설치 및 인증
**생성 파일:** 계획 아티팩트 기반의 풍부한 본문이 포함된 GitHub PR, STATE.md 업데이트

```bash
/redpill:ship 4                         # 페이즈 4 출시
/redpill:ship 4 --draft                 # 초안 PR로 출시
```
...
**사전 조건:** 프론트엔드 코드가 있는 프로젝트 (독립 실행 가능, REDPILL 프로젝트 불필요)
**생성 파일:** `{phase}-UI-REVIEW.md`, `.redpill/ui-reviews/`에 스크린샷

```bash
/redpill:ui-review                      # 현재 페이즈 감사
/redpill:ui-review 3                    # 페이즈 3 감사
```
...
**생성 파일:** `.redpill/reports/MILESTONE_SUMMARY-v{version}.md`
...
```bash
/redpill:milestone-summary                # 현재 마일스톤 요약
/redpill:milestone-summary v1.0           # 특정 마일스톤 요약
```
...
```bash
/redpill:new-milestone                  # 대화형 모드
/redpill:new-milestone "v2.0 Mobile"    # 이름이 지정된 마일스톤
/redpill:new-milestone --reset-phase-numbers "v2.0 Mobile"  # 마일스톤 번호를 1부터 재시작
```
...
```bash
/redpill:add-phase                      # 대화형 — 페이즈를 설명합니다
```
...
```bash
/redpill:insert-phase 3                 # 페이즈 3과 4 사이에 삽입 → 3.1 생성
```
...
```bash
/redpill:remove-phase 7                 # 페이즈 7 제거, 8→7, 9→8 등으로 재번호
```
...
```bash
/redpill:list-phase-assumptions 2       # 페이즈 2 가정 사항 확인
```
...
```bash
/redpill:plan-milestone-gaps             # 각 감사 갭에 대한 페이즈 생성
```

### `/redpill:research-phase`

심층 에코시스템 조사만 수행합니다 (독립 실행 — 일반적으로 `/redpill:plan-phase`를 사용하세요).
...
```bash
/redpill:research-phase 4               # 페이즈 4 도메인 조사
```
...
```bash
/redpill:validate-phase 2               # 페이즈 2 테스트 커버리지 감사
```
...
```bash
/redpill:progress                       # "지금 어디 있나? 다음은 무엇인가?"
```
...
```bash
/redpill:resume-work                    # 컨텍스트 초기화 또는 새 세션 후 실행
```
...
```bash
/redpill:pause-work                     # continue-here.md 생성
```
...
```bash
/redpill:quick                          # 기본 빠른 작업
/redpill:quick --discuss --research     # 토론 + 조사 + 계획
/redpill:quick --full                   # 계획 검사 및 검증 포함
/redpill:quick --discuss --research --full  # 모든 선택적 단계 포함
```
...
```bash
/redpill:autonomous                     # 남은 모든 페이즈 실행
/redpill:autonomous --from 3            # 페이즈 3부터 시작
```

### `/redpill:do`

자유 형식 텍스트를 적절한 REDPILL 명령어로 라우팅합니다.

```bash
/redpill:do                             # 원하는 작업을 설명합니다
```
...
```bash
/redpill:add-tests 2                    # 페이즈 2 테스트 생성
```
...
- `/redpill:dev-preferences` 명령어 — 모든 세션에서 선호도를 로드합니다
- `CLAUDE.md` 프로필 섹션 — Claude Code에 의해 자동으로 인식됩니다

```bash
/redpill:profile-user                   # 세션 분석 및 프로필 구축
/redpill:profile-user --questionnaire   # 대화형 설문지 대체 방법
/redpill:profile-user --refresh         # 새로운 분석으로 재생성
```

### `/redpill:health`

`.redpill/` 디렉터리의 무결성을 검사합니다.
...
```bash
/redpill:health                         # 무결성 검사
/redpill:health --repair                # 검사 및 수정
```
...
실패하거나 멈춘 REDPILL 워크플로우에 대한 사후 조사를 수행합니다.
...
**사전 조건:** `.redpill/` 디렉터리가 존재해야 합니다.
**생성 파일:** `.redpill/forensics/report-{timestamp}.md`
...
```bash
/redpill:forensics                              # 대화형 — 문제 입력 프롬프트
/redpill:forensics "Phase 3 execution stalled"  # 문제 설명과 함께 실행
```
...
**사전 조건:** 활성 REDPILL 프로젝트
**생성 파일:** `.redpill/` 하위의 워크스트림 디렉터리, 워크스트림별 상태 추적

```bash
/redpill:workstreams                    # 모든 워크스트림 목록
/redpill:workstreams create backend-api # 새 워크스트림 생성
/redpill:workstreams switch backend-api # 활성 워크스트림 설정
/redpill:workstreams status backend-api # 상세 상태 확인
/redpill:workstreams progress           # 교차 워크스트림 진행 상황 개요
/redpill:workstreams complete backend-api  # 완료된 워크스트림 아카이브
/redpill:workstreams resume backend-api    # 워크스트림 작업 재개
```
...
```bash
/redpill:set-profile budget             # 예산 프로필로 전환
/redpill:set-profile quality            # 품질 프로필로 전환
```
...
```bash
/redpill:map-codebase                   # 전체 코드베이스 분석
/redpill:map-codebase auth              # 인증 영역에 집중
```
...
```bash
/redpill:reapply-patches                # 로컬 변경사항 병합
```
...
**`/redpill:quick`의 대체가 아닙니다.** 조사, 다단계 계획 또는 검증이 필요한 작업에는 `/redpill:quick`을 사용하세요.
...
**생성 파일:** `{phase}-REVIEWS.md` — `/redpill:plan-phase --reviews`에서 사용 가능
...
`.redpill/` 커밋을 필터링한 깔끔한 PR 브랜치를 생성합니다.

| 인수 | 필수 여부 | 설명 |
|------|----------|------|
| `target branch` | 아니오 | 기본 브랜치 (기본값: `main`) |

**목적:** 리뷰어에게 REDPILL 계획 아티팩트가 아닌 코드 변경사항만 표시합니다.

```bash
/redpill:pr-branch                     # main을 기준으로 필터링
/redpill:pr-branch develop             # develop을 기준으로 필터링
```
...
**999.x 번호 체계**는 백로그 항목을 활성 페이즈 순서 밖에 유지합니다. 페이즈 디렉터리가 즉시 생성되므로 해당 항목에 대해 `/redpill:discuss-phase`와 `/redpill:plan-phase`를 사용할 수 있습니다.
...
**생성 파일:** `.redpill/seeds/SEED-NNN-slug.md`
**사용처:** `/redpill:new-milestone` (시드를 스캔하여 일치 항목 제시)
...
스레드는 여러 세션에 걸쳐 이어지지만 특정 페이즈에 속하지 않는 작업을 위한 경량 교차 세션 지식 저장소입니다. `/redpill:pause-work`보다 가볍습니다.

```bash
/redpill:thread                         # 모든 스레드 목록
/redpill:thread fix-deploy-key-auth     # 스레드 재개
/redpill:thread "Investigate TCP timeout in pasta service"  # 새 스레드 생성
```
