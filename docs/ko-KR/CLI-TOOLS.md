...
> `redpill-tools.cjs`에 대한 프로그래밍 방식 API 레퍼런스입니다. 워크플로우와 에이전트가 내부적으로 사용합니다. 사용자 대면 명령어는 [Command Reference](COMMANDS.md)를 참조하세요.

---

## 개요

`redpill-tools.cjs`는 GSD의 약 50개 명령어, 워크플로우, 에이전트 파일에서 반복되는 인라인 bash 패턴을 대체하는 Node.js CLI 유틸리티입니다. config 파싱, 모델 해석, 단계 조회, git 커밋, 요약 검증, 상태 관리, 템플릿 작업을 중앙화합니다.

**위치:** `redpill/bin/redpill-tools.cjs`
**모듈:** `redpill/bin/lib/`의 15개 도메인 모듈
...
`.redpill/STATE.md`를 관리합니다 — 프로젝트의 살아있는 메모리입니다.
...
에이전트 이름: `redpill-planner`, `redpill-executor`, `redpill-phase-researcher`, `redpill-project-researcher`, `redpill-research-synthesizer`, `redpill-verifier`, `redpill-plan-checker`, `redpill-integration-checker`, `redpill-roadmapper`, `redpill-debugger`, `redpill-codebase-mapper`, `redpill-nyquist-auditor`
