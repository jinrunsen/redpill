...
**목표:** `/redpill:new-project`가 `.redpill/config.json`을 생성할 때, 파일에 사용자가 선택한 6개 키만이 아닌 모든 유효한 기본값이 포함되도록 하여 개발자가 소스 코드를 읽지 않고도 모든 설정을 확인할 수 있게 합니다.
...
| 파일 | 액션 | 목적 |
|------|--------|---------|
| `redpill/bin/lib/config.cjs` | 수정 | `buildNewProjectConfig()` + `cmdConfigNewProject()` 추가 |
| `redpill/bin/redpill-tools.cjs` | 수정 | `config-new-project` case 등록 + usage 문자열 업데이트 |
| `redpill/workflows/new-project.md` | 수정 | Steps 2a + 5: 인라인 JSON 작성을 CLI 호출로 교체 |
...
`redpill/bin/lib/config.cjs`에서 `validateKnownConfigKeyPath` 함수 뒤(약 35번째 줄)와 `ensureConfigFile` 앞에 다음을 추가하세요:
...
/**
 * 명령어: 새 프로젝트를 위한 완전히 구체화된 .redpill/config.json을 생성합니다.
 *
 * 사용자가 선택한 설정을 JSON 문자열로 받습니다(/redpill:new-project 중 명시적으로
...
## 작업 2: redpill-tools.cjs에 `config-new-project` 등록

**파일.**

- 수정: `redpill/bin/redpill-tools.cjs`

- [ ] **Step 2.1: redpill-tools.cjs의 switch에 case 추가**
...
예상 결과: `{"created":true,"path":".redpill/config.json"}` (또는 유사한 형태)가 출력됩니다.
...
- 수정: `redpill/workflows/new-project.md`
...
**문제:**
`/redpill:new-project`는 온보딩 중 사용자가 명시적으로 선택한 6개 키만으로
`.redpill/config.json`을 생성합니다. 5개의 추가 키
...
2. **암묵적 확장**: `/redpill:settings` 또는 `config-set`이 처음으로 config에
   기록할 때도 해당 키들이 추가되지 않습니다. config는 유효한 구성의
   일부만 반영합니다.

**해결책:**
`redpill-tools.cjs`에 `config-new-project` CLI 명령어를 추가합니다. 이 명령어는:
...
**이것이 발견성을 개선하는 이유:**
처음으로 `.redpill/config.json`을 여는 개발자는 이제
`git.branching_strategy: "none"`을 확인하고 REDPILL 소스를 읽지 않고도
