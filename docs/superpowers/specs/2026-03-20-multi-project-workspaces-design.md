# Multi-Project Workspaces (`/redpill:new-workspace`)
...
GSD is tied to one `.redpill/` directory per working directory. Users with multiple independent projects (monorepo-style setups with 20+ child repos) or users needing feature branch isolation in the same repo cannot run parallel REDPILL sessions without manual cloning and state management.

## Solution

Three new commands that create, list, and remove **physical workspace directories** — each containing repo copies (git worktrees or clones) and an independent `.redpill/` directory.
...
Creates a workspace directory with repo copies and its own `.redpill/`.
...
Scans `~/gsd-workspaces/*/WORKSPACE.md` for workspace manifests. Displays table with name, path, repo count, REDPILL status (has PROJECT.md, current phase).
...
Key properties:
- `.redpill/` is at the workspace root, not inside any individual repo
- Each repo is a peer directory under the workspace root
- `WORKSPACE.md` is the only GSD-specific file at the root (besides `.redpill/`)
...
### `/redpill:new-workspace` Workflow Steps
...
7. **Initialize .redpill/** — `mkdir -p <workspace>/.planning`
8. **Offer /redpill:new-project** — Ask if user wants to run project initialization in the new workspace
...
| Workflow: new-workspace | `redpill/workflows/new-workspace.md` |
| Workflow: list-workspaces | `redpill/workflows/list-workspaces.md` |
| Workflow: remove-workspace | `redpill/workflows/remove-workspace.md` |
| Init function | `redpill/bin/lib/init.cjs` (add `cmdInitNewWorkspace`, `cmdInitListWorkspaces`, `cmdInitRemoveWorkspace`) |
| Routing | `redpill/bin/redpill-tools.cjs` (add cases to init switch) |
...
| `.redpill/` at workspace root | Gives full isolation from individual repo planning. Each workspace is an independent REDPILL project |
