...
Redpillフレームワークの包括的なドキュメントです。Redpill は、AI コーディングエージェント向けのメタプロンプティング、コンテキストエンジニアリング、仕様駆動開発システムです。
...
| [CLI ツールリファレンス](CLI-TOOLS.md) | コントリビューター、エージェント作成者 | `redpill-tools.cjs` のプログラマティック API（ワークフローおよびエージェント向け） |
...
- **はじめに:** [README](../README.md) → インストール → `/redpill:new-project`
