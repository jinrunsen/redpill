...
**前提条件:** 既存の `.redpill/PROJECT.md` がないこと
**生成物:** `PROJECT.md`、`REQUIREMENTS.md`、`ROADMAP.md`、`STATE.md`、`config.json`、`research/`、`CLAUDE.md`

```bash
/redpill:new-project                    # 対話モード
/redpill:new-project --auto @prd.md     # PRDから自動抽出
```
...
リポジトリのコピーと独立した `.redpill/` ディレクトリを持つ分離されたワークスペースを作成します。
...
**生成物:** `WORKSPACE.md`、`.redpill/`、リポジトリコピー（worktreeまたはclone）

```bash
/redpill:new-workspace --name feature-b --repos hr-ui,ZeymoAPI
/redpill:new-workspace --name feature-b --repos . --strategy worktree  # 同一リポジトリの分離
/redpill:new-workspace --name spike --repos api,web --strategy clone   # フルクローン
```
...
**前提条件:** `.redpill/ROADMAP.md` が存在すること
**生成物:** `{phase}-CONTEXT.md`、`{phase}-DISCUSSION-LOG.md`（監査証跡）

```bash
/redpill:discuss-phase 1                # フェーズ1の対話的ディスカッション
/redpill:discuss-phase 3 --auto         # フェーズ3でデフォルトを自動選択
/redpill:discuss-phase --batch          # 現在のフェーズのバッチモード
/redpill:discuss-phase 2 --analyze      # トレードオフ分析付きディスカッション
```
...
**前提条件:** `.redpill/ROADMAP.md` が存在し、フェーズにフロントエンド/UI作業があること
**生成物:** `{phase}-UI-SPEC.md`

```bash
/redpill:ui-phase 2                     # フェーズ2のデザイン契約書
```
...
```bash
/redpill:plan-phase 1                   # フェーズ1の調査＋計画＋検証
/redpill:plan-phase 3 --skip-research   # 調査なしで計画（馴染みのあるドメイン）
/redpill:plan-phase --auto              # 非対話型の計画
```
...
```bash
/redpill:execute-phase 1                # フェーズ1を実行
/redpill:execute-phase 1 --wave 2       # ウェーブ2のみを実行
```
...
```bash
/redpill:verify-work 1                  # フェーズ1のUAT
```
...
**前提条件:** `.redpill/` ディレクトリが存在すること
**動作:**
- プロジェクトなし → `/redpill:new-project` を提案
- フェーズにディスカッションが必要 → `/redpill:discuss-phase` を実行
- フェーズに計画が必要 → `/redpill:plan-phase` を実行
- フェーズに実行が必要 → `/redpill:execute-phase` を実行
- フェーズに検証が必要 → `/redpill:verify-work` を実行
- 全フェーズ完了 → `/redpill:complete-milestone` を提案
...
```bash
/redpill:session-report                 # セッション後のサマリーを生成
```
...
**前提条件:** フェーズが検証済み（`/redpill:verify-work` が合格）、`gh` CLIがインストールされ認証済みであること
**生成物:** 計画アーティファクトからリッチな本文を持つGitHub PR、STATE.mdの更新

```bash
/redpill:ship 4                         # フェーズ4をシップ
/redpill:ship 4 --draft                 # ドラフトPRとしてシップ
```
...
**生成物:** `{phase}-UI-REVIEW.md`、`.redpill/ui-reviews/` 内のスクリーンショット

```bash
/redpill:ui-review                      # 現在のフェーズを監査
/redpill:ui-review 3                    # フェーズ3を監査
```
...
**生成物:** `.redpill/reports/MILESTONE_SUMMARY-v{version}.md`
...
```bash
/redpill:milestone-summary                # 現在のマイルストーンをサマリー
/redpill:milestone-summary v1.0           # 特定のマイルストーンをサマリー
```
...
```bash
/redpill:new-milestone                  # 対話モード
/redpill:new-milestone "v2.0 Mobile"    # 名前付きマイルストーン
/redpill:new-milestone --reset-phase-numbers "v2.0 Mobile"  # マイルストーン番号を1からリスタート
```
...
```bash
/redpill:add-phase                      # 対話型 — フェーズの説明を入力
```
...
```bash
/redpill:insert-phase 3                 # フェーズ3と4の間に挿入 → 3.1を作成
```
...
```bash
/redpill:remove-phase 7                 # フェーズ7を削除、8→7、9→8等に番号振り直し
```
...
```bash
/redpill:list-phase-assumptions 2       # フェーズ2の前提を確認
```
...
```bash
/redpill:plan-milestone-gaps             # 各監査ギャップに対してフェーズを作成
```

### `/redpill:research-phase`

詳細なエコシステム調査のみを実行します（単体機能 — 通常は `/redpill:plan-phase` を使用してください）。
...
```bash
/redpill:research-phase 4               # フェーズ4のドメインを調査
```
...
```bash
/redpill:validate-phase 2               # フェーズ2のテストカバレッジを監査
```
...
```bash
/redpill:resume-work                    # コンテキストリセットまたは新しいセッション後に使用
```
...
```bash
/redpill:pause-work                     # continue-here.mdを作成
```
...
```bash
/redpill:manager                        # コマンドセンターダッシュボードを開く
```
...
```bash
/redpill:quick                          # 基本的なクイックタスク
/redpill:quick --discuss --research     # ディスカッション＋調査＋計画
/redpill:quick --full                   # プランチェックと検証付き
/redpill:quick --discuss --research --full  # すべてのオプションステージ
```
...
```bash
/redpill:autonomous                     # 残りの全フェーズを実行
/redpill:autonomous --from 3            # フェーズ3から開始
```
...
```bash
/redpill:add-tests 2                    # フェーズ2のテストを生成
```
...
```bash
/redpill:stats                          # プロジェクトメトリクスダッシュボード
```
...
- `/redpill:dev-preferences` コマンド — 任意のセッションでプリファレンスをロード
- `CLAUDE.md` プロファイルセクション — Claude Codeが自動検出

```bash
/redpill:profile-user                   # セッションを分析してプロファイルを構築
/redpill:profile-user --questionnaire   # 対話型アンケートのフォールバック
/redpill:profile-user --refresh         # 新鮮な分析からの再生成
```

### `/redpill:health`

`.redpill/` ディレクトリの整合性を検証します。
...
```bash
/redpill:health                         # 整合性チェック
/redpill:health --repair                # チェックして修復
```
...
**前提条件:** `.redpill/` ディレクトリが存在すること
**生成物:** `.redpill/forensics/report-{timestamp}.md`
...
```bash
/redpill:forensics                              # 対話型 — 問題の入力を促す
/redpill:forensics "Phase 3 execution stalled"  # 問題の説明付き
```
...
**生成物:** `.redpill/` 配下のワークストリームディレクトリ、ワークストリームごとの状態追跡

```bash
/redpill:workstreams                    # すべてのワークストリームを一覧表示
/redpill:workstreams create backend-api # 新しいワークストリームを作成
/redpill:workstreams switch backend-api # アクティブなワークストリームを設定
/redpill:workstreams status backend-api # 詳細ステータス
/redpill:workstreams progress           # ワークストリーム横断の進捗概要
/redpill:workstreams complete backend-api  # 完了したワークストリームをアーカイブ
/redpill:workstreams resume backend-api    # ワークストリームでの作業を再開
```
...
```bash
/redpill:set-profile budget             # budgetプロファイルに切り替え
/redpill:set-profile quality            # qualityプロファイルに切り替え
```
...
```bash
/redpill:map-codebase                   # コードベース全体を分析
/redpill:map-codebase auth              # auth領域にフォーカス
```
...
```bash
/redpill:update                         # アップデートを確認してインストール
```
...
```bash
/redpill:reapply-patches                # ローカルの変更をマージバック
```
...
**`/redpill:quick` の代替ではありません** — 調査、複数ステップの計画、または検証が必要な場合は `/redpill:quick` を使用してください。
...
**生成物:** `{phase}-REVIEWS.md` — `/redpill:plan-phase --reviews` で利用可能
...
`.redpill/` のコミットをフィルタリングしてクリーンなPRブランチを作成します。
...
```bash
/redpill:pr-branch                     # mainに対してフィルタリング
/redpill:pr-branch develop             # developに対してフィルタリング
```
...
**999.x番号付け**により、バックログ項目はアクティブなフェーズシーケンスの外に保持されます。フェーズディレクトリは即座に作成されるため、`/redpill:discuss-phase` や `/redpill:plan-phase` がそれらに対して動作します。
...
**生成物:** `.redpill/seeds/SEED-NNN-slug.md`
**利用先:** `/redpill:new-milestone`（シードをスキャンしてマッチするものを提示）
...
スレッドは、複数のセッションにまたがるが特定のフェーズに属さない作業のための軽量なクロスセッション知識ストアです。`/redpill:pause-work` よりも軽量です。

```bash
/redpill:thread                         # すべてのスレッドを一覧表示
/redpill:thread fix-deploy-key-auth     # スレッドを再開
/redpill:thread "Investigate TCP timeout in pasta service"  # 新規作成
```
