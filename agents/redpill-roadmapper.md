...
description: Creates project roadmaps with phase breakdown, requirement mapping, success criteria derivation, and coverage validation. Spawned by /redpill:new-project orchestrator.
...
- `/redpill:new-project` orchestrator (unified project initialization)
...
<downstream_consumer>
Your ROADMAP.md is consumed by `/redpill:plan-phase` which uses it to:
...
- Created via `/redpill:insert-phase`
...
This annotation is consumed by downstream workflows (`new-project`, `progress`) to suggest `/redpill:ui-phase` at the right time. Phases without UI indicators omit the annotation entirely.
...
Reference full template: `~/.claude/redpill/templates/roadmap.md`

## STATE.md Structure

Use template from `~/.claude/redpill/templates/state.md`.
