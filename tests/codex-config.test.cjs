/**
 * REDPILL Tools Tests - codex-config.cjs
 *
...
describe('getCodexSkillAdapterHeader', () => {
  ...
  test('includes correct invocation syntax', () => {
    ...
    assert.ok(result.includes('{{REDPILL_ARGS}}'), 'has REDPILL_ARGS variable');
  });
...
<role>
You are a REDPILL plan executor.
</role>`;
    ...
    assert.ok(result.includes('"redpill-executor"'), 'has quoted name');
    assert.ok(result.includes('"Executes REDPILL plans with atomic commits"'), 'has quoted description');
    ...
    assert.ok(result.includes('role: redpill-executor'), 'role matches agent name');
    assert.ok(result.includes('tools: Read, Write, Edit, Bash, Grep, Glob'), 'tools in role block');
    assert.ok(result.includes('purpose: Executes REDPILL plans with atomic commits'), 'purpose from description');
...
Run /redpill:execute-phase to proceed.`;

    const result = convertClaudeAgentToCodexAgent(input);
    assert.ok(result.includes('$gsd-execute-phase'), 'converts slash commands');
    assert.ok(!result.includes('/redpill:execute-phase'), 'original slash command removed');
  });
...
node "$HOME/.claude/redpill/bin/redpill-tools.cjs" commit "docs: resolve"`;

    const result = convertClaudeAgentToCodexAgent(input);
    assert.ok(result.includes('$HOME/.codex/redpill/bin/redpill-tools.cjs'), 'replaces $HOME/.claude/ with $HOME/.codex/');
...
<role>You check plans.</role>`;
  ...
  test('includes required name and description fields', () => {
    const result = generateCodexAgentToml('redpill-executor', sampleAgent);
    assert.ok(result.includes('name = "redpill-executor"'), 'has name');
...
describe('CODEX_AGENT_SANDBOX', () => {
  ...
  test('workspace-write agents have write tools', () => {
    const writeAgents = [
      'redpill-executor', 'redpill-planner', 'redpill-phase-researcher',
      'redpill-project-researcher', 'redpill-research-synthesizer', 'redpill-verifier',
      'redpill-codebase-mapper', 'redpill-roadmapper', 'redpill-debugger',
    ];
  ...
  test('read-only agents have no write tools', () => {
    const readOnlyAgents = ['redpill-plan-checker', 'redpill-integration-checker'];
...
describe('generateCodexConfigBlock', () => {
  const agents = [
    { name: 'redpill-executor', description: 'Executes plans' },
    { name: 'redpill-planner', description: 'Creates plans' },
  ];

  test('starts with REDPILL marker', () => {
    const result = generateCodexConfigBlock(agents);
    assert.ok(result.startsWith(REDPILL_CODEX_MARKER), 'starts with marker');
  });
  ...
  test('includes per-agent sections with relative paths (no targetDir)', () => {
    const result = generateCodexConfigBlock(agents);
    assert.ok(result.includes('[agents.redpill-executor]'), 'has executor section');
    assert.ok(result.includes('[agents.redpill-planner]'), 'has planner section');
    assert.ok(result.includes('config_file = "agents/redpill-executor.toml"'), 'relative config_file without targetDir');
  ...
  test('uses absolute config_file paths when targetDir is provided', () => {
    ...
    assert.ok(result.includes('config_file = "/home/user/.codex/agents/redpill-executor.toml"'), 'absolute executor path');
    assert.ok(result.includes('config_file = "/home/user/.codex/agents/redpill-planner.toml"'), 'absolute planner path');
...
describe('stripGsdFromCodexConfig', () => {
  test('returns null for GSD-only config', () => {
    const content = `${REDPILL_CODEX_MARKER}\n[features]\nmulti_agent = true\n`;
  ...
  test('preserves user content before marker', () => {
    ...
    assert.ok(!result.includes('multi_agent'), 'removes REDPILL content');
    assert.ok(!result.includes(REDPILL_CODEX_MARKER), 'removes marker');
  });
  ...
  test('strips injected keys above marker on uninstall', () => {
    ...
    assert.ok(!result.includes(REDPILL_CODEX_MARKER), 'strips marker');
  });

  test('removes [agents.gsd-*] sections', () => {
    const content = `[agents.redpill-executor]\ndescription = "test"\nconfig_file = "agents/redpill-executor.toml"\n\n[agents.custom-agent]\ndescription = "user agent"\n`;
    const result = stripGsdFromCodexConfig(content);
    assert.ok(!result.includes('[agents.redpill-executor]'), 'removes REDPILL agent section');
...
describe('mergeCodexConfig', () => {
  ...
  test('case 1: creates new config.toml', () => {
    ...
    assert.ok(content.includes(REDPILL_CODEX_MARKER), 'has marker');
    assert.ok(content.includes('[agents.redpill-executor]'), 'has agent');
    assert.ok(!content.includes('[features]'), 'no features section');
    assert.ok(!content.includes('multi_agent'), 'no multi_agent');
  });

  test('case 2: replaces existing REDPILL block', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    ...
    const newBlock = generateCodexConfigBlock([
      { name: 'redpill-executor', description: 'Updated description' },
      { name: 'redpill-planner', description: 'New agent' },
    ]);
    ...
    assert.ok(content.includes('[agents.redpill-planner]'), 'has new agent');
    // Verify no duplicate markers
    const markerCount = (content.match(new RegExp(REDPILL_CODEX_MARKER.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length;
    assert.strictEqual(markerCount, 1, 'exactly one marker');
  });

  test('case 3: appends to config without REDPILL marker', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    ...
    const content = fs.readFileSync(configPath, 'utf8');
    assert.ok(content.includes('[model]'), 'preserves user content');
    assert.ok(content.includes(REDPILL_CODEX_MARKER), 'adds marker');
    assert.ok(content.includes('[agents.redpill-executor]'), 'has agent');
  });

  test('case 3 with existing [features]: preserves user features, does not inject REDPILL keys', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    ...
    assert.ok(content.includes(REDPILL_CODEX_MARKER), 'adds marker for agents block');
    assert.ok(content.includes('[agents.redpill-executor]'), 'has agent');
  });
  ...
  test('case 3 strips existing [agents.gsd-*] sections before appending fresh block', () => {
    ...
    const content = fs.readFileSync(configPath, 'utf8');
    const gsdAgentCount = (content.match(/^\[agents\.redpill-executor\]\s*$/gm) || []).length;
    const markerCount = (content.match(new RegExp(REDPILL_CODEX_MARKER.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length;

    assert.ok(content.includes('[model]'), 'preserves user content');
    assert.ok(content.includes('[agents.custom-agent]'), 'preserves non-GSD agent section');
    assert.strictEqual(gsdAgentCount, 1, 'keeps exactly one REDPILL agent section');
    assert.strictEqual(markerCount, 1, 'adds exactly one marker block');
    assert.ok(!/\n{3,}# REDPILL Agent Configuration/.test(content), 'does not leave extra blank lines before marker block');
  });
  ...
  test('case 2 does not inject feature keys', () => {
    ...
    assert.ok(content.includes('[agents.redpill-executor]'), 'has agent from fresh block');
  });
  ...
  test('case 2 strips leaked GSD-managed sections above marker in CRLF files', () => {
    ...
    assert.strictEqual(countMatches(beforeMarker, /^\[agents\.redpill-executor\]\s*$/gm), 0, 'removes leaked REDPILL agent section above marker');
    assert.strictEqual(countMatches(content, /^\[agents\.redpill-executor\]\s*$/gm), 1, 'keeps one managed agent section');
    assertUsesOnlyEol(content, '\r\n');
  });

  test('case 2 preserves user-authored [agents] tables while stripping leaked REDPILL sections in CRLF files', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const brokenContent = [
      '[features]',
      'child_agents_md = false',
    ...
    assert.strictEqual(countMatches(beforeMarker, /^\[agents\.redpill-executor\]\s*$/gm), 0, 'removes leaked REDPILL agent section above marker');
    assert.strictEqual(countMatches(content, /^\[agents\.redpill-executor\]\s*$/gm), 1, 'keeps one managed agent section in the REDPILL block');
    assertUsesOnlyEol(content, '\r\n');
  });
  ...
  test('preserves CRLF when appending REDPILL block to existing config', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    fs.writeFileSync(configPath, '[model]\r\nname = "o3"\r\n', 'utf8');
    ...
    assert.ok(content.includes(`${REDPILL_CODEX_MARKER}\r\n`), 'writes marker with CRLF');
    assertUsesOnlyEol(content, '\r\n');
  });

  test('uses the first newline style when appending REDPILL block to mixed-EOL configs', () => {
    const configPath = path.join(tmpDir, 'config.toml');
...
describe('installCodexConfig (integration)', () => {
  ...
  (hasAgents ? test : test.skip)('generates config.toml and agent .toml files', () => {
    ...
    assert.ok(config.includes(REDPILL_CODEX_MARKER), 'has REDPILL marker');
    assert.ok(config.includes('[agents.redpill-executor]'), 'has executor agent');
    ...
    assert.ok(fs.existsSync(path.join(agentsDir, 'redpill-executor.toml')), 'executor .toml exists');
    assert.ok(fs.existsSync(path.join(agentsDir, 'redpill-plan-checker.toml')), 'plan-checker .toml exists');

    const executorToml = fs.readFileSync(path.join(agentsDir, 'redpill-executor.toml'), 'utf8');
    assert.ok(executorToml.includes('name = "redpill-executor"'), 'executor has name');
    assert.ok(executorToml.includes('description = "Executes REDPILL plans with atomic commits, deviation handling, checkpoint protocols, and state management. Spawned by execute-phase orchestrator or execute-plan command."'), 'executor has description');
    ...
    const checkerToml = fs.readFileSync(path.join(agentsDir, 'redpill-plan-checker.toml'), 'utf8');
    assert.ok(checkerToml.includes('name = "redpill-plan-checker"'), 'plan-checker has name');
...
describe('Codex install hook configuration (e2e)', () => {
  ...
  test('fresh CODEX_HOME enables codex_hooks without draft root defaults', () => {
    ...
    assert.ok(content.includes('# REDPILL Hooks\n[[hooks]]\nevent = "SessionStart"\n'), 'writes REDPILL SessionStart hook block');
    assert.strictEqual(countMatches(content, /^codex_hooks = true$/gm), 1, 'writes one codex_hooks key');
    assert.strictEqual(countMatches(content, /redpill-check-update\.js/g), 1, 'writes one REDPILL update hook');
  ...
  test('existing LF config without [features] gets one features block and preserves user content', () => {
    ...
    assert.strictEqual(countMatches(content, /redpill-check-update.js/g), 1, 'adds one REDPILL update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('bare top-level keys are NOT trapped under [features] (#1202)', () => {
    ...
    // followed by [projects] section. REDPILL must not prepend [features] before
  ...
  test('existing [features] at EOF without trailing newline is updated in place', () => {
    ...
    assert.ok(content.indexOf('codex_hooks = true') < content.indexOf('[agents.redpill-codebase-mapper]'), 'keeps codex_hooks before the next real table');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing empty [features] and codex_hooks = false are normalized and remain idempotent', () => {
    ...
    assert.ok(content.includes('command = "echo custom"'), 'preserves custom hook');
    assert.strictEqual(countMatches(content, /redpill-check-update.js/g), 1, 'does not duplicate REDPILL update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted [features] headers are recognized as the existing features table', () => {
    ...
    assert.strictEqual(countMatches(content, /redpill-check-update.js/g), 1, 'keeps one REDPILL update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted table headers containing # are parsed without treating # as a comment start', () => {
    ...
    assert.strictEqual(countMatches(content, /^codex_hooks = true$/gm), 1, 'adds one codex_hooks key');
    assert.strictEqual(countMatches(content, /redpill-check-update.js/g), 1, 'remains idempotent for the REDPILL hook block');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing dotted features config stays dotted and does not grow a [features] table', () => {
    ...
    assert.strictEqual(countMatches(content, /redpill-check-update.js/g), 1, 'adds one REDPILL update hook for dotted codex_hooks and remains idempotent');
    assertNoDraftRootKeys(content);
  });
  ...
  test('root inline-table features assignments are left untouched without appending invalid dotted keys or hooks', () => {
    ...
    assert.strictEqual(countMatches(content, /redpill-check-update.js/g), 0, 'does not add the REDPILL hook block when codex_hooks cannot be enabled safely');
    assert.ok(content.includes('[agents.redpill-executor]'), 'still installs the managed agent block');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted dotted codex_hooks keys stay dotted and are normalized without duplication', () => {
    ...
    assert.strictEqual(countMatches(content, /redpill-check-update.js/g), 1, 'adds one REDPILL update hook for quoted dotted codex_hooks and remains idempotent');
    assertNoDraftRootKeys(content);
  });
  ...
  test('codex_hooks = true with an inline comment is treated as enabled for hook installation', () => {
    ...
    assert.ok(content.includes('other_feature = true'), 'preserves other feature keys');
    assert.strictEqual(countMatches(content, /redpill-check-update.js/g), 1, 'adds the REDPILL update hook once');
    assertNoDraftRootKeys(content);
  });
  ...
  test('mixed-EOL configs use the first newline style for inserted Codex content', () => {
    ...
    assert.ok(content.includes(`# REDPILL Agent Configuration — managed by redpill installer\n`), 'writes the managed agent block using the first newline style');
    assert.ok(content.includes('# REDPILL Hooks\n[[hooks]]\nevent = "SessionStart"\n'), 'writes the REDPILL hook block using the first newline style');
    ...
    assert.strictEqual(countMatches(content, /redpill-check-update.js/g), 1, 'does not duplicate the REDPILL hook block');
    assertNoDraftRootKeys(content);
  });
});
...
describe('Codex uninstall symmetry for hook-enabled configs', () => {
  ...
  test('install then uninstall removes [features].codex_hooks while preserving other feature keys, comments, hooks, and CRLF', () => {
    ...
    assert.strictEqual(countMatches(cleaned, /redpill-check-update.js/g), 0, 'removes only the REDPILL update hook');
    assert.strictEqual(countMatches(cleaned, /\[agents\.gsd-/g), 0, 'removes managed REDPILL agent sections');
  ...
  test('install then uninstall removes dotted features.codex_hooks without creating a [features] table', () => {
    ...
    assert.strictEqual(countMatches(cleaned, /^features\.codex_hooks = true$/gm), 0, 'removes the dotted REDPILL codex_hooks key');
    assert.strictEqual(countMatches(cleaned, /^\[features\]\s*$/gm), 0, 'does not leave behind a [features] table');
    assert.strictEqual(countMatches(cleaned, /echo custom-after-command/g), 1, 'preserves non-GSD hooks');
    assert.strictEqual(countMatches(cleaned, /redpill-check-update.js/g), 0, 'removes the REDPILL update hook');
  });
  ...
  test('install then uninstall keeps mixed-EOL user content stable while removing REDPILL hook state', () => {
