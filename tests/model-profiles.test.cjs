...
} = require('../redpill/bin/lib/model-profiles.cjs');

// ─── MODEL_PROFILES data integrity ────────────────────────────────────────────

describe('MODEL_PROFILES', () => {
  test('contains all expected REDPILL agents', () => {
    const expectedAgents = [
      'redpill-planner', 'redpill-roadmapper', 'redpill-executor',
      'redpill-phase-researcher', 'redpill-project-researcher', 'redpill-research-synthesizer',
      'redpill-debugger', 'redpill-codebase-mapper', 'redpill-verifier',
      'redpill-plan-checker', 'redpill-integration-checker', 'redpill-nyquist-auditor',
      'redpill-ui-researcher', 'redpill-ui-checker', 'redpill-ui-auditor',
    ];
...
describe('VALID_PROFILES', () => {
  ...
  test('is derived from MODEL_PROFILES keys', () => {
    const fromData = Object.keys(MODEL_PROFILES['redpill-planner']);
...
describe('getAgentToModelMapForProfile', () => {
  test('returns correct models for balanced profile', () => {
    ...
    assert.strictEqual(map['redpill-planner'], 'opus');
    assert.strictEqual(map['redpill-codebase-mapper'], 'haiku');
    assert.strictEqual(map['redpill-verifier'], 'sonnet');
  });

  test('returns correct models for budget profile', () => {
    const map = getAgentToModelMapForProfile('budget');
    assert.strictEqual(map['redpill-planner'], 'sonnet');
    assert.strictEqual(map['redpill-phase-researcher'], 'haiku');
  });

  test('returns correct models for quality profile', () => {
    const map = getAgentToModelMapForProfile('quality');
    assert.strictEqual(map['redpill-planner'], 'opus');
    assert.strictEqual(map['redpill-executor'], 'opus');
  });
...
describe('formatAgentToModelMapAsTable', () => {
  test('produces a table with header and separator', () => {
    const map = { 'redpill-planner': 'opus', 'redpill-executor': 'sonnet' };
    ...
    assert.ok(table.includes('redpill-planner'), 'should list agent');
