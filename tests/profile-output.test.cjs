...
const {
  PROFILING_QUESTIONS,
  CLAUDE_INSTRUCTIONS,
} = require('../redpill/bin/lib/profile-output.cjs');
...
describe('write-profile command', () => {
  ...
  test('writes USER-PROFILE.md from analysis JSON', () => {
    ...
    const result = runRedpillTools(['write-profile', '--input', analysisPath, '--raw'], tmpDir);
  ...
  test('errors when --input is missing', () => {
    const result = runRedpillTools('write-profile --raw', tmpDir);
...
describe('generate-claude-md command', () => {
  ...
  test('generates CLAUDE.md with --auto flag', () => {
    ...
    const result = runRedpillTools(['generate-claude-md', '--output', outputPath, '--auto', '--raw'], tmpDir);
    assert.ok(result.success, `Failed: ${result.error}`);
...
describe('generate-dev-preferences command', () => {
  ...
  test('errors when --analysis is missing', () => {
    const result = runRedpillTools('generate-dev-preferences --raw', tmpDir);
  ...
  test('generates preferences from analysis file', () => {
    ...
    const result = runRedpillTools(['generate-dev-preferences', '--analysis', analysisPath, '--raw'], tmpDir);
    assert.ok(result.success, `Failed: ${result.error}`);
    const out = JSON.parse(result.output);
