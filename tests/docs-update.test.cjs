...
describe('docs-init command', () => {
  ...
  test('returns expected JSON shape', () => {
    const result = runRedpillTools(['docs-init'], tmpDir);
    ...
    assert.strictEqual(typeof data.redpill_dir_exists, 'boolean');
    ...
    // redpill_dir_exists is true since createTempProject creates .redpill/
    assert.strictEqual(data.redpill_dir_exists, true);
  });
...
describe('existing doc scanning', () => {
  ...
  test('detects REDPILL marker in existing docs', () => {
    fs.writeFileSync(
      path.join(tmpDir, 'README.md'),
      '<!-- generated-by: redpill-doc-writer -->\n# README\n',
      'utf-8'
    );
    ...
    assert.strictEqual(readmeEntry.has_gsd_marker, true, 'README.md should have REDPILL marker');

    const notesEntry = data.existing_docs.find(d => d.path === 'NOTES.md');
    assert.ok(notesEntry, 'NOTES.md should appear in existing_docs');
    assert.strictEqual(notesEntry.has_gsd_marker, false, 'NOTES.md should not have REDPILL marker');
  });
});
