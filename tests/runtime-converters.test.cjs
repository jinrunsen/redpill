...
// Sample Claude agent frontmatter (matches actual REDPILL agent format)
const SAMPLE_AGENT = `---
...
describe('OpenCode agent conversion (isAgent: true)', () => {
  test('keeps name: field for agents', () => {
    ...
    assert.ok(frontmatter.includes('name: redpill-executor'), 'name: should be preserved for agents');
  });
  ...
  test('strips skills: array', () => {
    ...
    assert.ok(!frontmatter.includes('redpill-executor-workflow'), 'skill entries should be stripped');
  });
  ...
  test('keeps description: field', () => {
    const result = convertClaudeToOpencodeFrontmatter(SAMPLE_AGENT, { isAgent: true });
    const frontmatter = result.split('---')[1];
    assert.ok(frontmatter.includes('description: Executes REDPILL plans'), 'description should be kept');
  });

  test('preserves body content', () => {
    const result = convertClaudeToOpencodeFrontmatter(SAMPLE_AGENT, { isAgent: true });
    assert.ok(result.includes('<role>'), 'body should be preserved');
    assert.ok(result.includes('You are a REDPILL plan executor.'), 'body content should be intact');
  });
...
Use \${PHASE} in shell examples.
    ...
    assert.ok(frontmatter.includes('name: redpill-codebase-mapper'), 'keeps name');
