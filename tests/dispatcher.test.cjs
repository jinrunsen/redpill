/**
 * REDPILL Tools Tests - Dispatcher
 *
 * Tests for redpill-tools.cjs dispatch routing and error paths.
...
describe('dispatcher error paths', () => {
  ...
  test('no-command invocation prints usage and exits non-zero', () => {
    const result = runRedpillTools('', tmpDir);
  ...
  test('unknown command produces clear error and exits non-zero', () => {
    const result = runRedpillTools('nonexistent-cmd', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('--cwd= form overrides working directory', () => {
    ...
    const result = runRedpillTools(`--cwd=${tmpDir} state load`, process.cwd());
  ...
  test('--cwd= with empty value produces error', () => {
    const result = runRedpillTools('--cwd= state load', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('--cwd with invalid path produces error', () => {
    const result = runRedpillTools('--cwd /nonexistent/path/xyz state load', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('template unknown subcommand errors', () => {
    const result = runRedpillTools('template bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('frontmatter unknown subcommand errors', () => {
    const result = runRedpillTools('frontmatter bogus file.md', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('verify unknown subcommand errors', () => {
    const result = runRedpillTools('verify bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('phases unknown subcommand errors', () => {
    const result = runRedpillTools('phases bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('roadmap unknown subcommand errors', () => {
    const result = runRedpillTools('roadmap bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('requirements unknown subcommand errors', () => {
    const result = runRedpillTools('requirements bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('phase unknown subcommand errors', () => {
    const result = runRedpillTools('phase bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('milestone unknown subcommand errors', () => {
    const result = runRedpillTools('milestone bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('validate unknown subcommand errors', () => {
    const result = runRedpillTools('validate bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('todo unknown subcommand errors', () => {
    const result = runRedpillTools('todo bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('uat unknown subcommand errors', () => {
    const result = runRedpillTools('uat bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('init unknown workflow errors', () => {
    const result = runRedpillTools('init bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
...
describe('dispatcher routing branches', () => {
  ...
  test('find-phase locates phase directory by number', () => {
    const phaseDir = path.join(tmpDir, '.redpill', 'phases', '01-test-phase');
  ...
  test('init resume returns valid JSON', () => {
    ...
    const result = runRedpillTools('init resume', tmpDir);
  ...
  test('init verify-work returns valid JSON', () => {
    ...
    const result = runRedpillTools('init verify-work 01', tmpDir);
  ...
  test('state with no subcommand calls cmdStateLoad', () => {
    ...
    const result = runRedpillTools('state', tmpDir);
...
`;
    ...
    const parsed = JSON.parse(result.output);
    assert.ok(typeof parsed === 'object', 'Output should be valid JSON object');
    assert.strictEqual(parsed.path, '.redpill/phases/01-test/01-01-SUMMARY.md', 'Path should match input');
