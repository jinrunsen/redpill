/**
 * REDPILL Tools Tests - Init BDD Phase
 *
...
describe('init bdd-phase', () => {
  ...
  test('errors when no phase argument provided', () => {
    const result = runRedpillTools('init bdd-phase', tmpDir);
  ...
  test('returns standard paths (state, roadmap, requirements)', () => {
    const phaseDir = path.join(tmpDir, '.redpill', 'phases', '03-auth');
    fs.mkdirSync(phaseDir, { recursive: true });

    const result = runRedpillTools('init bdd-phase 03', tmpDir);
  ...
  test('design_path found for NN-DESIGN.md naming', () => {
    ...
    const result = runRedpillTools('init bdd-phase 03', tmpDir);
    const output = JSON.parse(result.output);
    assert.strictEqual(output.design_path, '.redpill/phases/03-auth/03-DESIGN.md');
  });
  ...
  test('design_path found for bare DESIGN.md naming', () => {
    ...
    const result = runRedpillTools('init bdd-phase 03', tmpDir);
    const output = JSON.parse(result.output);
    assert.strictEqual(output.design_path, '.redpill/phases/03-auth/DESIGN.md');
  });
  ...
  test('has_bdd_progress is true and path returned when file exists', () => {
    ...
    assert.strictEqual(output.bdd_progress_path, '.redpill/phases/03-auth/BDD-PROGRESS.json');
  });
  ...
  test('has_bdd_progress defaults to false when phase directory does not exist', () => {
    ...
    const result = runRedpillTools('init bdd-phase 99', tmpDir);
