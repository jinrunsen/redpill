...
describe('phases list command', () => {
  ...
  test('empty phases directory returns empty array', () => {
    const result = runRedpillTools('phases list', tmpDir);
  ...
  test('lists phase directories sorted numerically', () => {
    ...
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '10-final'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '02-api'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '01-foundation'), { recursive: true });
  ...
  test('handles decimal phases in sort order', () => {
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '02-api'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '02.1-hotfix'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '02.2-patch'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '03-ui'), { recursive: true });
  ...
  test('--type plans lists only PLAN.md files', () => {
    const phaseDir = path.join(tmpDir, '.redpill', 'phases', '01-test');
    ...
    const result = runRedpillTools('phases list --type plans', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('--type summaries lists only SUMMARY.md files', () => {
    ...
    const result = runRedpillTools('phases list --type summaries', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('--phase filters to specific phase directory', () => {
    const phase01 = path.join(tmpDir, '.redpill', 'phases', '01-foundation');
    const phase02 = path.join(tmpDir, '.redpill', 'phases', '02-api');
    ...
    const result = runRedpillTools('phases list --type plans --phase 01', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('phase next-decimal command', () => {
  ...
  test('returns X.1 when no decimal phases exist', () => {
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '06-feature'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '07-next'), { recursive: true });

    const result = runRedpillTools('phase next-decimal 06', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('increments from existing decimal phases', () => {
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '06-feature'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '06.1-hotfix'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '06.2-patch'), { recursive: true });
  ...
  test('handles gaps in decimal sequence', () => {
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '06-feature'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '06.1-first'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '06.3-third'), { recursive: true });
  ...
  test('handles single-digit phase input', () => {
    ...
    const result = runRedpillTools('phase next-decimal 6', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error if base phase does not exist', () => {
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '01-start'), { recursive: true });
...
describe('phase-plan-index command', () => {
  ...
  test('empty phase directory returns empty plans array', () => {
    ...
    const result = runRedpillTools('phase-plan-index 03', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
  ...
  test('phase not found returns error', () => {
    const result = runRedpillTools('phase-plan-index 99', tmpDir);
    assert.ok(result.success, `Command should succeed: ${result.error}`);
...
describe('phase-plan-index canonical format', () => {
  ...
  test('files_modified: underscore key is parsed correctly', () => {
    const phaseDir = path.join(tmpDir, '.redpill', 'phases', '04-ui');
    fs.mkdirSync(phaseDir, { recursive: true });
...
`
    ...
    const result = runRedpillTools('phase-plan-index 04', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
  ...
  test('handles empty roadmap', () => {
    ...
    const result = runRedpillTools('phase add Initial Setup', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('phase add with project_code', () => {
  ...
  test('prefixes phase directory with project_code', () => {
    ...
    const result = runRedpillTools('phase add User Dashboard', tmpDir, { HOME: tmpDir });
    assert.ok(result.success, `Command failed: ${result.error}`);

    const output = JSON.parse(result.output);
    assert.strictEqual(output.phase_number, 2, 'should be phase 2');
    assert.ok(
      fs.existsSync(path.join(tmpDir, '.redpill', 'phases', 'CK-02-user-dashboard')),
  ...
  test('no prefix when project_code is null', () => {
    ...
    assert.ok(
      fs.existsSync(path.join(tmpDir, '.redpill', 'phases', '02-user-dashboard')),
      'directory should have no prefix'
    );
  });

  test('find-phase resolves prefixed directories', () => {
    const phaseDir = path.join(tmpDir, '.redpill', 'phases', 'CK-01-foundation');
    fs.mkdirSync(phaseDir, { recursive: true });
    fs.writeFileSync(path.join(phaseDir, '01-01-PLAN.md'), '# Plan');

    const result = runRedpillTools('find-phase 01', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('phases list sorts prefixed directories correctly', () => {
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', 'CK-02-api'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', 'CK-01-foundation'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', 'CK-03-ui'), { recursive: true });
...
`
    ...
    const result = runRedpillTools('phase insert 1 Fix Critical Bug', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    // Verify directory
    assert.ok(
      fs.existsSync(path.join(tmpDir, '.redpill', 'phases', '01.1-fix-critical-bug')),
...
`
    ...
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '01-foundation'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '01.1-hotfix'), { recursive: true });

    const result = runRedpillTools('phase insert 1 Another Fix', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('rejects missing phase', () => {
    ...
    const result = runRedpillTools('phase insert 99 Fix Something', tmpDir);
...
`
    ...
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '09.05-existing'), { recursive: true });

    // Pass unpadded "9.05" but roadmap has "09.05"
    const result = runRedpillTools('phase insert 9.05 Padding Test', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '05-feature-work'), { recursive: true });

    const result = runRedpillTools('phase insert 5 Hotfix', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '01-foundation'), { recursive: true });
    const p2 = path.join(tmpDir, '.redpill', 'phases', '02-auth');
    fs.mkdirSync(p2, { recursive: true });
    fs.writeFileSync(path.join(p2, '02-01-PLAN.md'), '# Plan');
    const p3 = path.join(tmpDir, '.redpill', 'phases', '03-features');
    fs.mkdirSync(p3, { recursive: true });
    fs.writeFileSync(path.join(p3, '03-01-PLAN.md'), '# Plan');
    fs.writeFileSync(path.join(p3, '03-02-PLAN.md'), '# Plan 2');

    // Remove phase 2
    const result = runRedpillTools('phase remove 2', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    assert.ok(
      fs.existsSync(path.join(tmpDir, '.redpill', 'phases', '02-features')),
      'phase 3 should be renumbered to 02-features'
    );
    assert.ok(
      !fs.existsSync(path.join(tmpDir, '.redpill', 'phases', '03-features')),
      'old 03-features should not exist'
    );

    // Files inside should be renamed
    assert.ok(
      fs.existsSync(path.join(tmpDir, '.redpill', 'phases', '02-features', '02-01-PLAN.md')),
      'plan file should be renumbered to 02-01'
    );
    assert.ok(
      fs.existsSync(path.join(tmpDir, '.redpill', 'phases', '02-features', '02-02-PLAN.md')),
  ...
  test('rejects removal of phase with summaries unless --force', () => {
    ...
    const result = runRedpillTools('phase remove 1', tmpDir);
    ...
    const forceResult = runRedpillTools('phase remove 1 --force', tmpDir);
  ...
  test('removes decimal phase and renumbers siblings', () => {
    ...
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '06-main'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '06.1-fix-a'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '06.2-fix-b'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '06.3-fix-c'), { recursive: true });

    const result = runRedpillTools('phase remove 6.2', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    // 06.3 should become 06.2
    assert.ok(
      fs.existsSync(path.join(tmpDir, '.redpill', 'phases', '06.2-fix-c')),
      '06.3 should be renumbered to 06.2'
    );
    assert.ok(
      !fs.existsSync(path.join(tmpDir, '.redpill', 'phases', '06.3-fix-c')),
  ...
  test('updates STATE.md phase count', () => {
    ...
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '02-b'), { recursive: true });

    runRedpillTools('phase remove 2', tmpDir);
...
`
  ...
  test('detects last phase in milestone', () => {
    ...
    const p1 = path.join(tmpDir, '.redpill', 'phases', '01-only-phase');
    fs.mkdirSync(p1, { recursive: true });
    fs.writeFileSync(path.join(p1, '01-01-PLAN.md'), '# Plan');
    fs.writeFileSync(path.join(p1, '01-01-SUMMARY.md'), '# Summary');
...
`
    ...
    fs.writeFileSync(
      path.join(tmpDir, '.redpill', 'REQUIREMENTS.md'),
      `# Requirements
...
`
    ...
    const p1 = path.join(tmpDir, '.redpill', 'phases', '01-auth');
    fs.mkdirSync(p1, { recursive: true });
    fs.writeFileSync(path.join(p1, '01-01-PLAN.md'), '# Plan');
    fs.writeFileSync(path.join(p1, '01-01-SUMMARY.md'), '# Summary');
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '02-api'), { recursive: true });
    ...
    const req = fs.readFileSync(path.join(tmpDir, '.redpill', 'REQUIREMENTS.md'), 'utf-8');
...
`
    ...
    const p1 = path.join(tmpDir, '.redpill', 'phases', '01-setup');
    fs.mkdirSync(p1, { recursive: true });
    fs.writeFileSync(path.join(p1, '01-01-PLAN.md'), '# Plan');
    fs.writeFileSync(path.join(p1, '01-01-SUMMARY.md'), '# Summary');
...
`
    ...
    fs.writeFileSync(path.join(p1, '01-01-PLAN.md'), '# Plan');
    fs.writeFileSync(path.join(p1, '01-01-SUMMARY.md'), '# Summary');
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '02-auth'), { recursive: true });
...
`
    ...
    const p32 = path.join(tmpDir, '.redpill', 'phases', '03.2-ipsum');
    const p321 = path.join(tmpDir, '.redpill', 'phases', '03.2.1-dolor-sit');
    const p4 = path.join(tmpDir, '.redpill', 'phases', '04-amet');
    ...
    const result = runRedpillTools('phase complete 03.2.1', tmpDir);
...
`
  ...
  test('updates STATE.md with plain format fields (no bold)', () => {
    ...
    const p1 = path.join(tmpDir, '.redpill', 'phases', '01-only');
    fs.mkdirSync(p1, { recursive: true });
    fs.writeFileSync(path.join(p1, '01-01-PLAN.md'), '# Plan');
    fs.writeFileSync(path.join(p1, '01-01-SUMMARY.md'), '# Summary');
...
const { comparePhaseNum, normalizePhaseName } = require('../redpill/bin/lib/core.cjs');
...
describe('letter-suffix phase sorting', () => {
  ...
  test('lists letter-suffix phases in correct order', () => {
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '12-foundation'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '12.1-inserted'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '12A-split'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '12A.1-bugfix'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '12B-hotfix'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '13-deploy'), { recursive: true });
...
describe('phase complete milestone-scoped next-phase', () => {
  ...
  test('finds next phase within milestone, ignoring prior milestone dirs', () => {
    ...
    for (let i = 1; i <= 4; i++) {
      ...
      const phaseDir = path.join(tmpDir, '.redpill', 'phases', `${padded}-old-phase`);
      fs.mkdirSync(phaseDir, { recursive: true });
    ...
    const p5 = path.join(tmpDir, '.redpill', 'phases', '05-auth');
    ...
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '06-dashboard'), { recursive: true });

    const result = runRedpillTools('phase complete 5', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
