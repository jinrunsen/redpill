...
const { runRedpillTools, createTempProject, createTempGitProject, cleanup } = require('./helpers.cjs');
...
describe('validate consistency command', () => {
  ...
  test('passes for consistent project', () => {
    ...
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '03-c'), { recursive: true });

    const result = runRedpillTools('validate consistency', tmpDir);
  ...
  test('warns about phase on disk but not in roadmap', () => {
    ...
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '01-a'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '02-orphan'), { recursive: true });
...
describe('verify plan-structure command', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject();
    fs.mkdirSync(path.join(tmpDir, '.redpill', 'phases', '01-test'), { recursive: true });
  });
  ...
  test('reports missing required frontmatter fields', () => {
    const planPath = path.join(tmpDir, '.redpill', 'phases', '01-test', '01-01-PLAN.md');
    fs.writeFileSync(planPath, '# No frontmatter here\n\nJust a plan without YAML.\n');

    const result = runRedpillTools('verify plan-structure .redpill/phases/01-test/01-01-PLAN.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error for nonexistent file', () => {
    const result = runRedpillTools('verify plan-structure .redpill/phases/01-test/nonexistent.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('verify phase-completeness command', () => {
  ...
  test('reports complete phase with matching plans and summaries', () => {
    ...
    const result = runRedpillTools('verify phase-completeness 01', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error for nonexistent phase', () => {
    const result = runRedpillTools('verify phase-completeness 99', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('verify summary command', () => {
  ...
  test('returns not found for nonexistent summary', () => {
    const result = runRedpillTools('verify-summary .redpill/phases/01-test/nonexistent.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('passes for valid summary with real files and commits', () => {
    ...
    const summaryPath = path.join(tmpDir, '.redpill', 'phases', '01-test', '01-01-SUMMARY.md');
    ...
    const result = runRedpillTools('verify-summary .redpill/phases/01-test/01-01-SUMMARY.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('respects checkFileCount parameter', () => {
    ...
    const result = runRedpillTools('verify-summary .redpill/phases/01-test/01-01-SUMMARY.md --check-count 1', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('verify references command', () => {
  ...
  test('reports valid when all referenced files exist', () => {
    ...
    const filePath = path.join(tmpDir, '.redpill', 'phases', '01-test', 'doc.md');
    fs.writeFileSync(filePath, '@src/app.js\n');

    const result = runRedpillTools('verify references .redpill/phases/01-test/doc.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error for nonexistent file', () => {
    const result = runRedpillTools('verify references .redpill/phases/01-test/nonexistent.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('verify commits command', () => {
  ...
  test('validates real commit hashes', () => {
    ...
    const result = runRedpillTools(`verify commits ${hash}`, tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('reports invalid for fake hashes', () => {
    const result = runRedpillTools('verify commits abcdef1234567', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('handles mixed valid and invalid hashes', () => {
    ...
    const result = runRedpillTools(`verify commits ${hash} abcdef1234567`, tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('verify artifacts command', () => {
  ...
  test('passes when all artifacts exist and match criteria', () => {
    ...
    const result = runRedpillTools('verify artifacts .redpill/phases/01-test/01-01-PLAN.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('verify key-links command', () => {
  ...
  test('verifies link when pattern found in source', () => {
    ...
    const result = runRedpillTools('verify key-links .redpill/phases/01-test/01-01-PLAN.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
