/**
 * REDPILL Tools Tests - Init Manager
 */
...
function writeRoadmap(tmpDir, phases) {
  ...
  fs.writeFileSync(path.join(tmpDir, '.redpill', 'ROADMAP.md'), content);
}

// Helper: write a minimal STATE.md
function writeState(tmpDir) {
  fs.writeFileSync(path.join(tmpDir, '.redpill', 'STATE.md'), '---\nstatus: active\n---\n# State\n');
}
...
function scaffoldPhase(tmpDir, num, opts = {}) {
  ...
  const dir = path.join(tmpDir, '.redpill', 'phases', `${padded}-${slug}`);
...
describe('init manager', () => {
  ...
  test('fails without ROADMAP.md', () => {
    writeState(tmpDir);
    const result = runRedpillTools('init manager', tmpDir);
  ...
  test('WAITING.json detected when present', () => {
    ...
    fs.writeFileSync(
      path.join(tmpDir, '.redpill', 'WAITING.json'),
