/**
 * REDPILL Tools Tests - Agent Skills Injection
 *
 * CLI integration tests for the `agent-skills` command that reads
 * `agent_skills` from .redpill/config.json and returns a formatted
...
describe('agent-skills command', () => {
  ...
  test('returns empty when no config exists', () => {
    ...
    const result = runRedpillTools(['agent-skills', 'redpill-executor'], tmpDir, { HOME: tmpDir, USERPROFILE: tmpDir });
  ...
  test('returns empty for unconfigured agent type', () => {
    writeConfig(tmpDir, {
      agent_skills: {
        'redpill-executor': ['skills/test-skill'],
      },
    });
    const result = runRedpillTools(['agent-skills', 'redpill-planner'], tmpDir, { HOME: tmpDir, USERPROFILE: tmpDir });
    assert.strictEqual(result.output, '');
  });
  ...
  test('returns formatted block for configured agent with single string path', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: {
        'redpill-executor': 'skills/my-skill',
      },
    });
  ...
  test('handles multiple skill paths', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: {
        'redpill-executor': ['skills/skill-a', 'skills/skill-b'],
      },
    });
  ...
  test('warns for nonexistent skill path but does not error', () => {
    writeConfig(tmpDir, {
      agent_skills: {
        'redpill-executor': ['skills/nonexistent'],
      },
    });
  ...
  test('returns empty when no agent type argument provided', () => {
    const result = runRedpillTools(['agent-skills'], tmpDir, { HOME: tmpDir, USERPROFILE: tmpDir });
...
describe('config-set agent_skills', () => {
  ...
  test('can set agent_skills via dot notation', () => {
    const result = runRedpillTools(
      ['config-set', 'agent_skills.redpill-executor', '["skills/my-skill"]'],
      tmpDir,
    ...
    assert.deepStrictEqual(
      config.agent_skills['redpill-executor'],
      ['skills/my-skill'],
